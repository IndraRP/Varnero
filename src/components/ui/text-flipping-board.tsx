...
const FlapCell = React.memo(function FlapCell({
  ...
  return (
    <div className="flex aspect-3/6 flex-col overflow-hidden rounded-[2px] border md:rounded-[3px] md:border-2 border-black">
      {/* Flap content area */}
      <div className="relative flex-1 perspective-dramatic transform-3d">
        ...
        {flipId > 0 && (
          ...
          >
            ...
            <motion.div
              className="pointer-events-none absolute inset-0 not-first:bg-[linear-gradient(to_top,rgba(0,0,0,0),rgba(0,0,0,0.6))]"
              initial={{ opacity: 0.4 }}
              animate={{ opacity: 0 }}
              transition={{
                duration: flipDuration * 0.85,
      ...
      <div className="h-2 w-full bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.15rem)] mask-t-from-50% md:h-4 md:bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.2rem)] text-black opacity-100" />
    </div>
  );
},
(prevProps, nextProps) =>
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-3xl rounded-xlp-2 md:rounded-2xl md:p-4 bg-neutral-900 shadow-[0_20px_70px_-15px_rgba(0,0,0,0.6)]",
        className,
      )}
    >
      <div
